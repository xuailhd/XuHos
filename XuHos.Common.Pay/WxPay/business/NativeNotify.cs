...
namespace KMEHosp.Common.Pay.WxPay
{
    ...
    /// </summary>
    internal class NativeNotify:Notify
    {
        ...
        private WxPayData UnifiedOrder(string openId,string productId, string appId)
        {
            var WxPayConfig = Configuration.GetAppPayConfig<Config.Sections.Pay.WXPay>(appId);
            ...
            req.SetValue("out_trade_no", WxPayApi.GenerateOutTradeNo(WxPayConfig.MCHID));
            ...
            WxPayData result = WxPayApi.UnifiedOrder(req,6,appId);
            return result;
        }
    }
}
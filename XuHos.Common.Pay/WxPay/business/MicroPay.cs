...
namespace KMEHosp.Common.Pay.WxPay
{
    internal class MicroPay
    {
        ...
        * @return 刷卡支付结果
        */
        public static string Run(string body, string total_fee, string auth_code, string appId)
        {
            ...
            data.SetValue("out_trade_no", WxPayApi.GenerateOutTradeNo(WxPayConfig.MCHID));//产生随机的商户订单号

            WxPayData result = WxPayApi.Micropay(data, 10,appId); //提交被扫支付，接收返回结果
		    ...
		    while(queryTimes-- > 0)
		    {
			    ...
			    WxPayData queryResult = Query(out_trade_no, out succResult,appId);
			    //如果需要继续查询，则等待2s后继续
			    if(succResult == 2)
                {
				    Thread.Sleep(2000);
		    ...
		    if(!Cancel(out_trade_no,0,appId))
		    {
	    ...
	    */
        public static WxPayData Query(string out_trade_no, out int succCode,string appId)
	    {
		    WxPayData queryOrderInput = new WxPayData();
		    queryOrderInput.SetValue("out_trade_no",out_trade_no);
		    WxPayData result = WxPayApi.OrderQuery(queryOrderInput,6,appId);
	    ...
	    */
	    public static bool Cancel(string out_trade_no, int depth,string appId)
	    {
		    if(depth > 10)
            {
			    return false;
		    ...
		    WxPayData result = WxPayApi.Reverse(reverseInput,5,appId);
            ...
            {
			    return Cancel(out_trade_no, ++depth,appId);
		    }
		    return false;
	    }
    }

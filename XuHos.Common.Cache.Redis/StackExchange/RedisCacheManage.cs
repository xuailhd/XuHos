...
namespace XuHos.Common.Cache.Redis.StackExchangeImplement
{
    public static class CacheConfigHelper
    {
        /// <summary>
        /// 分割服务器列表
        /// </summary>
    ...
    public class RedisCacheManage : ICacheManager
    {
        ...
        //Redis集群分片存储定位器
        private static XuHos.Common.KetamaHash.KetamaNodeLocator _Locator;
        ...
        /// 创建链接池管理对象
        /// </summary>
        public static RedisCacheManage Create(XuHos.Common.Config.Sections.Redis config, int DbNum = 0)
        {
                        ...
                        if (string.IsNullOrEmpty(config.SentineList) || !_supportSentinal)
                        {
                            ...
                            if (writeServerArray.Count == 1)
                            {
                                ...
                                if (!_clusterConfigOptions.ContainsKey(NodeName))
                                {
                                    ...
                                    configOption.DefaultDatabase = string.IsNullOrEmpty(config.DBNum) ? 0 : int.Parse(config.DBNum);
                                    configOption.Ssl = config.Ssl != null && bool.TrueString.ToUpper() == config.Ssl.ToUpper() ? true : false;
                            ...
                            */
                                ...
                                for (int i = 0; i < writeServerArray.Count; i++)
                                {
                                    ...
                                    if (writeServerArray[i].IndexOf("@") > 0)
                                    {
                                        ...
                                        if (!_clusterConfigOptions.ContainsKey(NodeName))
                                        {
                                            ...
                                            configOption.Password = config.Password;
                                            configOption.AbortOnConnectFail = false;
                                            configOption.DefaultDatabase = string.IsNullOrEmpty(config.DBNum) ? int.Parse(config.DBNum) : 0;
                                            configOption.Ssl = config.Ssl != null && bool.TrueString.ToUpper() == config.Ssl.ToUpper() ? true : false;
                            ...
                            _Locator = new XuHos.Common.KetamaHash.KetamaNodeLocator(Nodes, _VIRTUAL_NODE_COUNT);
                        }
                        else
                        {
                            ...
                            for (int i = 0; i < SentineList.Count; i++)
                            {
                                ...
                                if (!_clusterConfigOptions.ContainsKey(hostName))
                                {
                                    ...
                                    sentinelConfig.DefaultDatabase = string.IsNullOrEmpty(config.DBNum) ? int.Parse(config.DBNum) : 0;
                            ...
                            //初始化Reds分片定位器
                            _Locator = new XuHos.Common.KetamaHash.KetamaNodeLocator(sentinelServerHostList, _VIRTUAL_NODE_COUNT);
                        }
                    }
                }
            }

            return new RedisCacheManage(DbNum);
        }
